...
| **`재구축_가이드.md`** | **인스턴스 삭제 후 처음부터 다시 ← 여기부터** |
| `UI_구조설명.md` | 웹 UI 를 어떻게 만들었는지 (Gradio) |
| `실행순서.md` | 최소 구성 실행 순서 |
...
| **`교수님_질문_답변.md`** | **영상 길이 제한 / 프레임 조절 / 벤치마크 3종 · 구축 방식 (근거 인용)** |
...
프레임 수는 `--max-frames` 로도 직접 제한할 수 있습니다 (미지정 시 768 = fps2 기준 384초).
자세한 근거는 `교수님_질문_답변.md` 참고.
